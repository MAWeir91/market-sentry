...
def _has_local_json_preflight_arg(raw_argv: Sequence[str]) -> bool:
    return any(
        ...
        or item == "--local-rvol-artifact-manifest-write"
...
def _artifact_manifest_writer_conflicts(
    raw_argv: Sequence[str],
    args: argparse.Namespace,
) -> list[str]:
    conflict_flags = {
...
def _render_local_rvol_artifact_manifest_writer_conflict_error(
    command: LocalRvolArtifactManifestWriterCommandRequest,
    conflicts: Sequence[str],
) -> str:
    manifest_path = command.output_path if command.output_path is not None else "N/A"
    return "\n".join(
        [
...
def _build_local_rvol_artifact_manifest_writer_command_request(
    args: argparse.Namespace,
) -> LocalRvolArtifactManifestWriterCommandRequest:
...
def main(
    ...
    if writer_path is not None or writer_artifacts is not None:
        command = _build_local_rvol_artifact_manifest_writer_command_request(args)
        conflicts = _artifact_manifest_writer_conflicts(raw_argv, args)
        if conflicts:
            print(
                _render_local_rvol_artifact_manifest_writer_conflict_error(
                    command,
                    conflicts,
                )
            )
            return 2
        if writer_path is None and writer_artifacts is not None:
            print(
                render_local_rvol_artifact_manifest_writer_command_error(
                    command,
                    LocalRvolArtifactManifestWriterCommandError(
        ...
        try:
            result = run_local_rvol_artifact_manifest_writer(command)
        except (
            LocalRvolArtifactManifestWriterCommandError,
            LocalRvolArtifactManifestWriteError,
        ) as exc:
            print(render_local_rvol_artifact_manifest_writer_command_error(command, exc))
            return 2
        except OSError as exc:
            print(render_local_rvol_artifact_manifest_writer_error(command, exc))
            return 1
